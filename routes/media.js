...
router.post('/', [
    ...
    check('anioestreno', 'El año de estreno es obligatorio y debe ser un número').isNumeric(),
    ...
    try {
        ...
        if (!genero) {
            return res.status(400).json({ msg: 'El género seleccionado no está activo o no existe' });
        }

        if (!director) {
            return res.status(400).json({ msg: 'El director seleccionado no está activo o no existe' });
        }

        if (!productora) {
            return res.status(400).json({ msg: 'La productora seleccionada no está activa o no existe' });
        }
...
router.get('/:id', async (req, res) => {
    try {
        const media = await Media.findById(req.params.id)
            .populate('generoprincipal directorprincipal productora tipo');
